...
2. link:examples/package.html[Packaging] an application into different formats: fat JAR, native image, Docker, JKube, etc.
3. link:dockerfile-mojo.html[Generating `Dockerfile`]s.
4. link:examples/deploy.html[Deploying] Docker or JKube-managed images to different registries.
...
The `<packaging>` part is important since the plugin support several packaging types. The default is `jar`, and
supported Micronaut packagings include `docker`, `jkube-k8s`, `jkube-oc`, `docker-native`, and `docker-crac`.

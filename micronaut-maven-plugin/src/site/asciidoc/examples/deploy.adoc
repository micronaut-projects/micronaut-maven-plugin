...
* `native-image-jib`: writes a daemonless tarball during `package` by default, or publishes during `package` when `-Djib.buildGoal=build` is set.
...
For daemonless native executable images, use `package` with `native-image-jib` and Jib's registry build goal:

----
$ mvn package -Dpackaging=native-image-jib -Djib.buildGoal=build -Djib.to.image=my-company/my-app:latest
----

This path builds the local native executable, copies only that executable into an OCI image, and pushes with Jib registry
credentials. It does not bind `mvn deploy -Dpackaging=native-image-jib` to a separate push goal.
...
For `native-image-jib` packaging, omitting `jib.buildGoal` produces `target/jib-image.tar` by default, and

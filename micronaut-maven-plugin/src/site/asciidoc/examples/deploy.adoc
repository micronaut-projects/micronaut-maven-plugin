...
For daemonless native executable images, use `package` with `native-image`, enable the native image Jib workflow, and set Jib's registry build goal:
...
credentials. It does not bind `mvn deploy -Dpackaging=native-image` to a separate push goal.

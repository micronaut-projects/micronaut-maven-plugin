...
* `buildpack`: builds a local Cloud Native Buildpacks image during `package`; registry publish is not mapped to `deploy`
...
Cloud Native Buildpacks packaging is currently a local image creation path. Running `mvn package -Dpackaging=buildpack`
invokes `pack build` with the configured `micronaut.buildpack.*` properties, but `mvn deploy -Dpackaging=buildpack` does
not publish the image to a registry. Use `docker` packaging with `-Djib.buildGoal=build`, `native-image` with
`micronaut.native-image.jib.enabled=true` and `-Djib.buildGoal=build`, or the JKube `k8s`/`openshift` packaging paths when
you need plugin-managed registry publication from Maven.

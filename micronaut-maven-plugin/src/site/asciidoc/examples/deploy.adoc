...
* `jkube-k8s` or `jkube-oc`: will delegate image push to the matching Eclipse JKube plugin.
...
For JKube-based deployments, declare the corresponding JKube plugin in your POM and use the JKube packaging:
...
Then invoke either `mvn deploy -Dpackaging=jkube-k8s` or `mvn deploy -Dpackaging=jkube-oc`.
Micronaut Maven Plugin binds those packagings to `k8s:push` and `oc:push` respectively, while the actual registry,
cluster, and manifest behavior remains owned by JKube.
...
Docker-specific helper goals such as `mn:dockerfile` and `mn:docker-push` are still limited to Docker packagings and
should not be used with `jkube-k8s` or `jkube-oc`.

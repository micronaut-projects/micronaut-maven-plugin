...
* `jkube-k8s` or `jkube-oc`: will delegate image publication to Eclipse JKube's `push` goal for the selected platform.
...
----
$ mvn deploy -Dpackaging=jkube-k8s
$ mvn deploy -Dpackaging=jkube-oc
----

`jkube-k8s` maps `deploy` to `org.eclipse.jkube:kubernetes-maven-plugin:push`, while `jkube-oc` maps `deploy` to

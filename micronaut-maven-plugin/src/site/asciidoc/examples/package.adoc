...
If there is a `Dockerfile` in the project's root directory, it will be copied into `target/` and used to build the
native image with `target/` as the Docker build context.

The plugin still prepares the native-image inputs under `target/` and passes the same build arguments as the bundled
native Dockerfiles where applicable. Depending on the runtime and image mode, it may provide:

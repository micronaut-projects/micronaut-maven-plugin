...
* `jkube-k8s`: delegates packaging to Eclipse JKube's Kubernetes Maven Plugin (`k8s:build`).
* `jkube-oc`: delegates packaging to Eclipse JKube's OpenShift Maven Plugin (`oc:build`).
...
The `jkube-k8s` and `jkube-oc` packaging types let Micronaut's lifecycle bindings delegate to Eclipse JKube instead of
the built-in Docker/Jib packaging.

* `jkube-k8s` maps `package` to `org.eclipse.jkube:kubernetes-maven-plugin:build`.
* `jkube-oc` maps `package` to `org.eclipse.jkube:openshift-maven-plugin:build`.

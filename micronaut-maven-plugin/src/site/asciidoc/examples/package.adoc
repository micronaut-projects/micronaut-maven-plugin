...
of the build steps expect a certain base image. By default, the native images are built from an `ghcr.io/graalvm/native-image` image.

In the case of AWS custom runtime, it starts from `amazonlinux:latest`, and this cannot be changed. Also, in this case the

...
* `native-image-jib`: generates a GraalVM native image and packages it into an OCI image with Jib, without a Docker daemon.
...
----
$ mvn package -Dpackaging=native-image-jib
----

If the `<packaging>` is set to `native-image-jib`, this plugin first delegates to
`org.graalvm.buildtools:native-maven-plugin:compile-no-fork` to create the local native executable, then packages only
that executable into an OCI image with Jib Core. This workflow does not require a Docker daemon.

By default, `native-image-jib` writes the Jib tarball to `target/jib-image.tar`:
...
[source,shell]
----
mvn package -Dpackaging=native-image-jib -Djib.buildGoal=build -Djib.to.image=my-company/my-app:latest
----

The supported `jib.buildGoal` values for `native-image-jib` are `buildTar` and `build`. Use `docker-native` packaging
...
`native-image-jib` packages a locally built executable into a Linux container image. By default, it fails on non-Linux

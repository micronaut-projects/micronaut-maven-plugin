...
If your custom `Dockerfile` needs those values, declare the matching `ARG` instructions and copy the generated native
build inputs from the `target/` context. For example:

...
Configuration validation goals are wired when using Micronaut packaging types (`jar`, `native-image`, `docker`, `docker-native`, `docker-crac`, `jkube-k8s`, `jkube-oc`), but validation only executes when explicitly enabled:

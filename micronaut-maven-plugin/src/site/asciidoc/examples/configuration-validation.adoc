...
Configuration validation goals are wired when using Micronaut packaging types (`jar`, `native-image`, `docker`, `docker-native`, `docker-crac`, `k8s`, `openshift`), but validation only executes when explicitly enabled:

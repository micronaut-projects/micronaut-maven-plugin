...
public abstract class AbstractDockerMojo extends AbstractMicronautMojo {
     ...
     * but can be set to <code>build</code> to build the image without a Docker daemon. Note that Jib's <code>build</code>
     * goal with also push the image to a container registry. Check the

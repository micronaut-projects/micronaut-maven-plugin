...
Refer to the
https://graalvm.github.io/native-build-tools/0.9.7.1/[Native Maven Plugin documentation]
...
building a native-image. This is a https://graalvm.github.io/native-build-tools/0.9.7.1/maven-plugin.html#long_classpath_and_shading_support[known issue]
with Windows. To workaround this you need to configure the Shade plugin in your `pom.xml` and then configure the native image

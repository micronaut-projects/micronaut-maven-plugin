...
public class DockerNativeMojo extends AbstractMojo {
    ...
    @Inject
    public DockerNativeMojo(MavenProject mavenProject, MavenSession mavenSession) {
    ...
    public void execute() {
        ...
        if (DOCKER_NATIVE_PACKAGING.equals(packaging)) {
            try {
                ...
                //TODO check Jib plugin config?
                Set<String> tags = new HashSet<>(Collections.singletonList(mavenProject.getName()));

                String imageId = dockerClient.buildImageCmd(dockerfile)
                        //TODO allow to customize, check Jib plugin config
                        //TODO check Java version
                        .withBuildArg("BASE_IMAGE", "oracle/graalvm-ce:20.2.0-java11")
